 ...
 *
 * Theme-aware by construction, not by a light/dark swap: the ink is
 * `currentColor`, so it automatically matches whatever text color is active
 * wherever the logo sits (near-black in light mode, near-white in dark,
 * same as the surrounding copy). The "paper" cutouts (the notches in the
 * bangs, the face, the eraser highlight) default to the page background —
 * pass `paper` when the logo sits on a different surface (a card, a tinted
 * section) so those cutouts don't show a mismatched patch of color. */
export function Logo({
  ...
  return (
    <svg viewBox="0 0 300 320" aria-hidden="true" className={className}>
      ...
      <circle cx="80" cy="249" r="8" fill={paper} stroke="currentColor" strokeWidth="6" />
