...
/** Disconnects Google Calendar — deletes the stored tokens. Existing
 * CalendarSyncLink rows are left as-is; harmless since every write path in
 * src/lib/google-calendar.ts already no-ops when there's no connection, and
 * reconnecting later reuses the same links instead of duplicating events. */
