...
export async function createAssignment(
  ...
  let assignment: { id: string; title: string; dueDate: Date | null };
...
export async function updateAssignment(
  ...
  after(() =>
    dueDate
      ...
      : deleteCalendarEvent({ userId, sourceType: "assignment", sourceId: assignmentId }),
  );
...
export async function deleteAssignment(
  ...
  after(() => deleteCalendarEvent({ userId, sourceType: "assignment", sourceId: assignmentId }));
...
export async function setAssignmentCompleted(
  ...
  const assignment = await db.assignment.updateManyAndReturn({
    where: { id: assignmentId, courseId, course: { userId } },
    data: { completed },
    select: { id: true, title: true, dueDate: true },
  });

  // A completed assignment drops off the calendar (nothing left to do by
  // that date); un-completing it re-adds the event if it still has a date.
  after(() => {
    const row = assignment[0];
    if (!row) {
      return Promise.resolve();
    }
    if (completed || !row.dueDate) {
      return deleteCalendarEvent({ userId, sourceType: "assignment", sourceId: assignmentId });
    }
...
export async function createExam(
  ...
  let exam: { id: string; title: string; examDate: Date };
...
export async function deleteExam(courseId: string, examId: string): Promise<void> {
  ...
  after(() => deleteCalendarEvent({ userId, sourceType: "exam", sourceId: examId }));
