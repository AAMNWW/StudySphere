...
export default async function CareerLayout({
  ...
  const [resumes, jobApplications, careerChatThreads, interviewSessions] = await Promise.all([
    db.resume.count({ where: { userId } }),
    db.jobApplication.count({ where: { userId } }),
    db.careerChatThread.count({ where: { userId } }),
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-6 px-6 py-10 md:flex-row md:gap-8">
      <CareerSidebar
        counts={{ resumes, jobApplications, careerChatThreads, interviewSessions }}
      />
